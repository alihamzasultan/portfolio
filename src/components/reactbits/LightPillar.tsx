...
const LightPillar = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        col = customTanh(col * uGlowAmount / widthNorm);
  ...
  if (!webGLSupported) {
    return (
      <div className={className} style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.1)', color: '#888', mixBlendMode: mixBlendMode as any }}>
        WebGL not supported
      </div>
    );
  }

  return <div ref={containerRef} className={className} style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0, mixBlendMode: mixBlendMode as any }} />;
};
